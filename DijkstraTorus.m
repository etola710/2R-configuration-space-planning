...
while true
    ...
    for q=1:length(neighbors)
        try
            if neighbors(q,1) > nrows
                neighbors(q,1) = 1;
            elseif neighbors(q,1) < 1
                neighbors(q,1) = nrows;
            end
            if neighbors(q,2) > ncols
                neighbors(q,2) = 1;
            elseif neighbors(q,2) < 1
                neighbors(q,2) = ncols;
            end
            neighbor_ind = sub2ind(size(distances), neighbors(q,1),neighbors(q,2));
            neighbor_dist = distances(neighbor_ind);
